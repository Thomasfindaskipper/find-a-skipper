...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (profileError || !data) {
        setError(profileError?.message || 'Impossible de charger votre profil.');
        setLoading(false);
        return;
      }
  ...
  async function saveProfile(markDone: boolean) {
    ...
    if (markDone && previewProfile && !isRoleProfileReady(previewProfile)) {
      setSaving(false);
      setError('Merci de completer les informations requises avant de terminer.');
      return;
    }
  ...
  return (
    <main className="max-w-xl mx-auto px-6 py-10">
      <h1 className="font-display text-3xl font-bold mb-2">Finaliser votre profil</h1>
      <p className="text-sm text-gray-500 mb-6">
        Quelques informations sont necessaires pour activer votre espace {profile.role === 'skipper' ? 'skipper' : 'demandeur'}.
      </p>
      <ErrorBanner message={error} />

      <div className="rounded-2xl p-5 mb-6 bg-white border border-navy/[0.08]">
        <h2 className="font-semibold mb-2">Elements manquants</h2>
        {missing.length === 0 ? (
          <p className="text-sm text-emerald-700">Tout est pret, vous pouvez terminer l&apos;onboarding.</p>
        ) : (
        ...
        {profile.role === 'skipper' && (
          <>
            <Field label="Zones de navigation">
              <div>{ZONES.map((zone) => <Tag key={zone} label={zone} active={zones.includes(zone)} onClick={() => toggle(zones, setZones, zone)} />)}</div>
            </Field>
            <Field label="Types de bateaux maitrises">
              <div>{BOAT_TYPES.map((boat) => <Tag key={boat} label={boat} active={boatTypes.includes(boat)} onClick={() => toggle(boatTypes, setBoatTypes, boat)} />)}</div>
            </Field>
